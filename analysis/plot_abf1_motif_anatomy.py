...
def main():
    ...
    sites = [("site #3  —  upstream of ERV46   chrI:61163-61177", chrI[61164:61164 + L],
              "REJECTED by the sequence layer   (posterior 0.922 -> 0.032)"),
             ("site #4  —  downstream of ERV46  chrI:62657-62671", chrI[62658:62658 + L],
              "KEPT by the sequence layer       (posterior 0.708 -> 0.9998)")]
    ...
    fig.suptitle("Why the sequence layer rejects the ABF1 site upstream of ERV46",
                 fontsize=15, fontweight="bold", y=0.995)
    fig.text(0.5, 0.963,
             "Both sites have near-perfect ABF1 half-sites. The whole difference is the "
             "5 spacer bases — which ABF1 does not read, but this fixed-width PWM scores anyway.",
             ha="center", fontsize=10.2, color=MUTED)
    plt.tight_layout(rect=[0, 0.045, 1, 0.952])
