...
def fiber_track(outDir, chrm, lo, hi):
    """Raw Fiber-seq m6A per base over [lo,hi]. Returns dict or None.
    Watson channel is read at reference A, Crick at reference T -- exactly the
    positions layers 5/6 use; G/C carry no m6A information at all."""
    try:
        co = pd.read_csv(os.path.join(outDir, "coords.tsv"), sep="\t")
    ...
    k = np.where(isA, kw[i], np.where(isT, kc[i], 0)).astype(float)
    n = np.where(isA, nw[i], np.where(isT, nc[i], 0)).astype(float)
    frac = np.where(n > 0, k / np.maximum(n, 1), np.nan)
    # coverage-weighted rolling means, computed SEPARATELY per strand channel
    W = 15
    def roll(mask):
...
def main():
    ...
    args = ap.parse_args()
    out = args.out or ("abf1_5sites_%s_withfiber.png" % args.key)
    ...
    fig = plt.figure(figsize=(13.6, 5.0 * n))
    # outer grid = one cell per site (generous gap for the annotation rows that
    # overhang the top of each posterior panel); inner grid keeps each
    # posterior/fiber pair glued together so the x axes line up visually.
    outer = gridspec.GridSpec(n, 1, hspace=0.42)
    axes, waxes, caxes = [], [], []
    for r in range(n):
        inner = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=outer[r],
                                                 height_ratios=[3.0, 1.0, 1.0], hspace=0.06)
        a = fig.add_subplot(inner[0]); axes.append(a)
        waxes.append(fig.add_subplot(inner[1], sharex=a))
        caxes.append(fig.add_subplot(inner[2], sharex=a))
    summary = []

    for ax, wax, cax, (num, chrm, m0, m1, name), det in zip(axes, waxes, caxes, SITES, details):
        mid = (m0 + m1) // 2
        ...
        # same convention as plotRoboCOP.plotFiberseqAx: two axes, blue / orange, 0-1
        ax.tick_params(labelbottom=False)
        ft = fiber_track(args.outdir, chrm, lo - 5, hi + 5)
        for sax, col, sel, smk, lab in (
            ...
            sax.set_yticklabels(["0.0", "0.5", ""])   # blank the top label: it collides with the panel above
