...
def _tf_pads(d):
    """VARIANT widenull: per-TF (left, right) Fiber-seq footprint pads, motif frame.

    Falls back to the legacy symmetric scalar d['padding'] so a BASELINE HMMconfig.pkl
    (which has 'padding' but no 'tf_pads') still decodes correctly under this variant --
    that fallback is what lets score_robocop.py --pkg point here for every run.
    """
    p = d.get('tf_pads')
    if p is not None:
        return np.asarray(p, dtype=int)
    return np.full((d['n_tfs'], 2), int(d.get('padding', 0)), dtype=int)


def read_tf_pads(tfs, path=None):
    """Load analysis/tf_pads.tsv into an (n_tfs, 2) array aligned to `tfs`.

    Read ONCE, at train time, and frozen into HMMconfig.pkl as dshared['tf_pads'].
    Decodes inherit it from there and never re-read the file, so editing the tsv can
    never desynchronise a decode from the trainDir whose tf_lens it depends on.
    """
    if path is None:
        path = os.environ.get('ROBOCOP_TF_PADS', 'tf_pads.tsv')
    if not os.path.isfile(path):
        raise IOError(
            "VARIANT widenull needs a pad table but %s does not exist. Create it (see "
            "analysis/tf_pads.tsv) or set ROBOCOP_TF_PADS." % path)
    spec = {}
    for ln, raw in enumerate(open(path), 1):
        ...
        f = line.split()
        if len(f) != 3:
            raise ValueError("%s:%d: expected '<tf> <left> <right>'" % (path, ln))
        spec[f[0]] = (int(f[1]), int(f[2]))
    tfs = list(tfs)
    unknown = [t for t in spec if t not in tfs]
    if unknown:
        raise ValueError("%s lists TFs absent from the model: %s" % (path, unknown))
    pads = np.array([spec.get(t, (0, 0)) for t in tfs], dtype=int)
    print("tf_pads (%s): %s" % (path, {t: spec[t] for t in sorted(spec)} or 'none'))
    return pads
...
def createSharedDictionary(d, fasta_file, nucleosome_file, tf_prob, background_prob, nucleosome_prob, pwm, tmpDir, info_file, nt_array = None, n_obs = None, lock = None):
    ...
    d['padding'] = 0 # legacy symmetric padding -- superseded by tf_pads, kept at 0
    # VARIANT widenull: per-TF asymmetric footprint pads, in the motif's own 5'->3' frame.
    d['tf_pads'] = read_tf_pads(tfs)
    d['tmpDir'] = tmpDir
...
def get_transition_matrix_info(d, pwm, allow_end_at_any_state):
    ...
    _pads = _tf_pads(d)
    if d['nucleotides'] is None:
        tf_lens = np.array([10 + _pads[i][0] + _pads[i][1] for i in range(len(tfs))])
    else:
        tf_lens = np.array([pwm[tfs[i]].shape[1] + _pads[i][0] + _pads[i][1]
                            for i in range(len(tfs))]) # default TF length 10
...
def stack_pwms(d, pwm, nuc_emission):
    ...
    _pads = _tf_pads(d)
    bg_col = np.transpose(pwm['background'])
    for _i, tf in enumerate(tfs):
        pl, pr = int(_pads[_i][0]), int(_pads[_i][1])
        # Forward block: pl pad cols, motif, pr pad cols.
        # Pad columns emit pwm['background'] -- the SAME vector the background state
        # emits -- so they cancel in the TF-vs-background likelihood ratio and the
        # sequence layer's contribution stays exactly the motif log-odds.
        for i in range(pl): pwm_emat = np.vstack((pwm_emat, bg_col))
        pwm_emat = np.vstack((pwm_emat, np.transpose(pwm[tf])))
        for i in range(pr): pwm_emat = np.vstack((pwm_emat, bg_col))

        # Reverse block: pads MIRROR, exactly as reverse_complement mirrors the PWM,
        # so the motif-frame right pad becomes the reference-left pad. Getting this
        # order wrong shifts every minus-strand site by (pl - pr) bp.
        for i in range(pr): pwm_emat = np.vstack((pwm_emat, bg_col))
...
def update_data_emission_matrix_using_fiber_seq_counts_Bionomial(
    ...
    with open('inputs/all_TFs_1000pealVal_params_pseudo_widenull.pkl', 'rb') as f:  # VARIANT widenull: per-TF widened footprints
        loaded_params = pickle.load(f)
    ...
    for i in range(dshared['n_tfs']):
        ...
        if tf_name in loaded_params['p']:
            ...
            if len(p_forward) + len(p_reverse) != tf_end - tf_start:
                raise ValueError(
                    "VARIANT widenull: %s p-vector is %d bp but its state block is %d bp. The "
                    "params pkl and the trainDir's tf_lens disagree -- rebuild one of them "
                    "(make_params_wide.py / train_wide.py) from the same tf_pads.tsv."
                    % (tf_name, len(p_forward), (tf_end - tf_start) // 2))
    ...
    for t in range(dshared['timepoints']):
        ...
        for i in range(n_obs):
            ...
            for j in range(dshared['n_tfs']):
                # motif centre, pad-aware (VARIANT widenull)
                _pl, _pr = int(_cpads[j][0]), int(_cpads[j][1])
                _core = tf_lens[j] - _pl - _pr
                dbf_binding_probs[i, j + 1] = posterior_table[i, int(tf_starts[j] + _pl + _core//2)]
                # reverse motif -- pads mirror, so the leading pad there is _pr
                dbf_binding_probs[i, j + 1] += posterior_table[i, int(tf_starts[j] + tf_lens[j] + _pr + _core//2)]
            if dshared['nuc_present']:
...
def sum_for_dbf_probs(dshared, posterior_table):
    ...
    tf_starts = dshared['tf_starts']
    tf_lens = dshared['tf_lens']
    # VARIANT widenull: MOTIF-CORE-ONLY sum. Pad states are excluded, so the reported
    # posterior stays a motif-width peak and remains directly comparable to every
    # unwidened run. Bounds are hoisted out of the per-position loop.
    #   forward block = [pl pad][motif][pr pad]
    #   reverse block = [pr pad][motif][pl pad]      (pads mirror, as in stack_pwms)
    # NB the upstream reverse slice omitted its leading pad; invisible at padding=0.
    _pads = _tf_pads(dshared)
    _fs = [tf_starts[j] + _pads[j][0] for j in range(dshared['n_tfs'])]
    _fe = [tf_starts[j] + tf_lens[j] - _pads[j][1] for j in range(dshared['n_tfs'])]
    _rs = [tf_starts[j] + tf_lens[j] + _pads[j][1] for j in range(dshared['n_tfs'])]
    _re = [tf_starts[j] + 2*tf_lens[j] - _pads[j][0] for j in range(dshared['n_tfs'])]
    for t in range(dshared['timepoints']):
        dbf_binding_probs = np.zeros((n_obs, dshared['n_tfs'] + 1 + 5*dshared['nuc_present']))
        for i in range(n_obs):
            # background
            ...
            for j in range(dshared['n_tfs']):
                # motif
                dbf_binding_probs[i, j + 1] = posterior_table[i, _fs[j]:_fe[j]].sum()
                # reverse motif
                dbf_binding_probs[i, j + 1] += posterior_table[i, _rs[j]:_re[j]].sum()
            if dshared['nuc_present']:
                dbf_binding_probs[i, dshared['n_tfs'] + 1] = \
                                                       posterior_table[i, nuc_start:nuc_padding_end1].sum() + \
                                                       posterior_table[i, nuc_padding_start2:nuc_padding_end2].sum()
...
def sum_for_dbf_probs_fwd_rev(d, dshared):
    ...
    for t in range(dshared['timepoints']):
        ...
        for i in range(d['n_obs']):
            ...
            for j in range(dshared['n_tfs']):
                # motif core only (VARIANT widenull)
                dbf_binding_probs[i, j + 1] = posterior_table[i, (tf_starts[j] + _fpads[j][0]):(tf_starts[j] + tf_lens[j] - _fpads[j][1])].sum()
