...
def posterior_forward_backward_loop(d, dshared, segment):
    ...
    # EM10-OFF: # After posterior_decoding, data_emission_mat is a NumPy array
    # EM10-OFF: # shape: (7, n_obs, n_states)
    # EM10-OFF: save_dir = os.path.join(dshared["tmpDir"], "emission_plots")
    # EM10-OFF: os.makedirs(save_dir, exist_ok=True)
    # EM10-OFF: for layer in range(data_emission_mat.shape[0]):
        # EM10-OFF: layer_data = data_emission_mat[layer].ravel()
        # EM10-OFF: finite_vals = layer_data[np.isfinite(layer_data)]
        # EM10-OFF: if finite_vals.size == 0:
            # EM10-OFF: print(f"Layer {layer}: no finite values, skipping")
            # EM10-OFF: continue
        # EM10-OFF: # Compute summary stats
        # EM10-OFF: vmin = finite_vals.min()
        # EM10-OFF: vmax = finite_vals.max()
        # EM10-OFF: vmedian = np.median(finite_vals)
        # EM10-OFF: fig, axes = plt.subplots(1, 2, figsize=(10, 4))
        # EM10-OFF: # --- Histogram ---
        # EM10-OFF: axes[0].hist(finite_vals, bins=100, color='steelblue', alpha=0.7, label=f"min={vmin:.3e}")
        # EM10-OFF: axes[0].set_title(f"Histogram – layer {layer}")
        # EM10-OFF: axes[0].set_xlabel("Emission value")
        # EM10-OFF: axes[0].set_ylabel("Frequency")
        # EM10-OFF: axes[0].set_yscale("log")
        # EM10-OFF: axes[0].legend()
        # EM10-OFF: # --- Box plot ---
        # EM10-OFF: axes[1].boxplot(finite_vals, vert=True)
        # EM10-OFF: axes[1].set_title(f"Layer {layer} | min={vmin:.3e}, median={vmedian:.3e}, max={vmax:.3e}")
        # EM10-OFF: plt.tight_layout()
        # EM10-OFF: plt.savefig(os.path.join(save_dir, f"emission_layer_{layer}.png"), dpi=150)
        # EM10-OFF: plt.close(fig)
    # import sys
