...
#     robocop.update_data_emission_matrix_using_fiber_seq_counts_onePhi(d, t, dshared, nuc_phi = countParams['nucShort']['phi'], nuc_mus = countParams['nucShort']['mu']*countParams['nucShort']['scale'], tf_phi = countParams['tfShort']['phi'], tf_mu = countParams['tfShort']['mu'], other_phi = countParams['otherShort']['phi'], other_mu = countParams['otherShort']['mu'], FiberType = 'crick', tech = tech)
  ...
  ## Sets the sequence emission to uniform 1 value so that it doesn't interfere with the results 
    ...
    ## SEQUENCE-ONLY variant: neutralise BOTH Fiber channels. Emission is a PRODUCT
    ## over layers, so 1.0 contributes nothing -- this is the exact mirror of the
    ## `data_emission_matrix[0][:] = 1` line the fiber-only variants use.
    ## Must come AFTER the 1e-30 floor above, otherwise the floor would re-populate
    ## these layers with 1e-30 and reintroduce a (tiny, uniform) fiber term.
    data_emission_matrix[5][:] = 1
    data_emission_matrix[6][:] = 1

    ## ABF1 HARD-forbid mask on LAYER 0, not on 5/6.
    ## The other variants put this mask on the Fiber channels, but here those are
    ## all 1.0 -- masking them would be silently wiped by the two lines above and the
    ## run would quietly become a full 154-TF decode. Layer 0 is the only live layer
    ## in this variant, so the mask has to ride on it. Same semantics: a 0 in any
    ## channel zeroes the product, so states 29..nuc_start (all non-ABF1 TFs incl.
    ## 'unknown') get posterior exactly 0, while background (0), ABF1 (1..28) and
    ## nucleosomes (nuc_start..) keep their PWM emission.
    data_emission_matrix[0][:, 29:dshared['nuc_start']] = 0
