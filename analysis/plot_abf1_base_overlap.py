...
CASES = [
    (3, "site #3   upstream of ERV46   —  FAILS when both layers are on",
        dict(start=61156, rev=True), dict(start=61164, rev=False)),
    (4, "site #4   downstream of ERV46  —  WORKS",
        dict(start=62656, rev=False), dict(start=62658, rev=False)),
]
...
def bases(d, start, rev, L=14):
    ...
    Pw = d["PW"][::-1] if rev else d["PW"]
    Pc = d["PC"][::-1] if rev else d["PC"]
    out = []
    for j in range(L):
...
def main():
    d = load()
    fig, axes = plt.subplots(len(CASES), 1, figsize=(15.2, 5.6 * len(CASES)))
    summary = []
            ...
            for x in bb:
                ...
                if x["ch"]:
                    ax.text(x["pos"], y - 0.10, "%d/%d" % (x["k"], x["n"]), ha="center",
                            va="center", fontsize=7.4, color=MUTED, zorder=4)
                    big = x["f"] >= 50
                    ax.text(x["pos"], y - 0.26, ("%.0f×" % x["f"]) if x["f"] >= 10 else ("%.1f×" % x["f"]),
                            ha="center", va="center", fontsize=8.4 if big else 7.6,
    ...
    fig.suptitle("Which bases each ABF1 placement actually uses  —  k/n = methylated / fibers covering that base",
                 fontsize=14.5, fontweight="bold", y=0.998)
    fig.text(0.5, 0.972,
             "Coverage is flat across the region (43–82 fibers per base). What changes with the shift is WHICH "
             "bases fall inside the 14 bp window and WHICH expected value each one is matched against.",
             ha="center", fontsize=10, color=MUTED)
    plt.tight_layout(rect=[0, 0.035, 1, 0.962])
